...
class LightMemory:
    def __init__(self, config: BaseMemoryConfigs = BaseMemoryConfigs()):
        ...
        if self.config.topic_segment:
            ...
            self.segmenter = TopicSegmenterFactory.from_config(self.config.topic_segmenter, self.config.precomp_topic_shared, compressor)
    ...
    def add_memory(
        ...
        *,
        ...
        if self.config.metadata_generate:
            ...
            # No LLM extraction: build minimal pass-through results from segments for memory entries
            extracted_results = []
                ...
                for seg in batch:
                    ...
                    for msg in seg:
                        if msg.get("role") == "user" and msg.get("content"):
                            facts.append({"fact": msg["content"], "source_id": 0})
                    if facts:
                        cleaned.append(facts)
                extracted_results.append({"cleaned_result": cleaned} if cleaned else {"cleaned_result": [[]]})
