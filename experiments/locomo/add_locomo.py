...
def process_single_sample(sample, api_key, args):
    ...
    try:
        ...
        logger.error(f"[FAIL] {sample_id} failed: {str(e)}", exc_info=True)
        return {
            'sample_id': sample_id,
            'status': 'failed',
            'error': str(e)
...
def main():
    ...
    for sample in data:
        ...
        main_logger.info(f"[SKIP] {sample_id}: Needs processing ({', '.join(status)})")
            ...
            for future in as_completed(future_to_sample):
                ...
                try:
                    ...
                    if result['status'] == 'success':
                        ...
                        pbar.set_postfix_str(f"[FAIL] {result['sample_id']}")
