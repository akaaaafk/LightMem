...
TINKER_MODEL = os.getenv("TINKER_MODEL", "Qwen/Qwen3-30B-A3B-Instruct-2507")
...
class TinkerLLMModel:

    def __init__(self, api_key: str, base_model: str, max_tokens: int = 2000, temperature: float = 0.0):
        self.api_key = api_key
        ...
        self._sampling_client = self._service.create_sampling_client(base_model=base_model)
    ...
    def call(self, messages: list, **kwargs):
        ...
        for attempt in range(max_retries):
            try:
                ...
                prompt = self.types.ModelInput.from_ints(input_ids)
                ...
                samples = getattr(result, "samples", None) or getattr(result, "sequences", [])
                if not samples:
                    return ""
                out_tokens = samples[0].tokens
                response = self._tokenizer.decode(out_tokens, skip_special_tokens=True)
                print(response)
                return response
            except Exception as e:
                print(f"[Retry {attempt + 1}/{max_retries}]  {type(e).__name__}: {e}")
...
llm = TinkerLLMModel(TINKER_API_KEY, TINKER_MODEL)
