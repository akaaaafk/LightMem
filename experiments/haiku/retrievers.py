...
class QdrantEntryLoader:
    ...
    def _fallback_sqlite_read(self, collection_name: str, with_vectors: bool = True) -> List[Dict[str, Any]]:
        ...
        if not os.path.exists(storage_sqlite):
            storage_sqlite = os.path.join(self.qdrant_path, 'collection', collection_name, 'storage.sqlite')
        if not os.path.exists(storage_sqlite):
            logger.error(f"SQLite file not found for collection {collection_name} (tried nested and collection/ paths)")
            return []
...
class VectorRetriever:
    ...
    def retrieve(self, entries: List[Dict], query_text: str, limit: int = 20) -> List[Dict]:
        ...
        valid_entries = [e for e in entries if e.get('vector') is not None]
        ...
        matrix = np.array([e['vector'] for e in valid_entries], dtype=np.float32)
        q = np.array(query_vector, dtype=np.float32)
        q_norm = np.linalg.norm(q)
        row_norms = np.linalg.norm(matrix, axis=1)
        if q_norm == 0:
            scores = np.zeros(len(valid_entries), dtype=np.float32)
        else:
            denom = row_norms * q_norm
            denom = np.where(denom == 0, 1e-12, denom)
            scores = matrix.dot(q) / denom
        ...
        return [
            {
                'id': str(valid_entries[i].get('id')),
                'score': float(scores[i]),
                'payload': valid_entries[i].get('payload', {}),
                'source': 'vector',
            }
            for i in top_idx
        ]
