...
def evaluate_llm_judge(question, gold_answer, generated_answer, client_obj=None, model_name="gpt-4o-mini"):
    if client_obj is None:
        raise ValueError("client_obj must be provided; no default client is configured.")
    ...
    try:
        label = json.loads(extract_json(raw))["label"]
    except (json.JSONDecodeError, KeyError):
        label = "CORRECT" if "CORRECT" in raw.upper() else "WRONG"
